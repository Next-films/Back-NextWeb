...
import {
  TelegramAdminBotSendNotificationNewModerationMovieCommand,
  TelegramAdminBotSendNotificationNewModerationMovieCommandHandler,
} from '@/telegram/admin-bot/application/handlers/bot-send-notification-new-moderation-movie.handler';
import { MovieDurationUtil } from '@/common/utils/movie-duration.util';

describe('TelegramAdminBotSendNotificationNewModerationMovieCommandHandler (integration)', () => {
  let app: INestApplication;
  let handler: TelegramAdminBotSendNotificationNewModerationMovieCommandHandler;
  ...
  let moderation_movie_front_url: string;
  ...
  beforeAll(async () => {
    ...
    handler = app.get(TelegramAdminBotSendNotificationNewModerationMovieCommandHandler);
    ...
    moderation_movie_front_url = businessRulesSettings.MODERATION_MOVIE_FRONT_URL;
  });
  ...
  it('should send notification about new moderation task', async () => {
    ...
    const expectedData1: BotNewMovieForModerationPayloadDto = {
      movieId: 1,
      title: `Movie 0`,
      duration: MovieDurationUtil.formatDuration(0),
      releaseDate: 'unknown',
      originalTitle: 'unknown',
      link: `${moderation_movie_front_url}/${MovieTypesEnum.FILM}/${1}`,
      type: telegramAdminBotService.getMovieTypeString(MovieTypesEnum.FILM),
    };

    const expectedData2: BotNewMovieForModerationPayloadDto = {
      movieId: 1,
      title: `Movie 0`,
      duration: MovieDurationUtil.formatDuration(0),
      releaseDate: 'unknown',
      originalTitle: 'unknown',
      link: `${moderation_movie_front_url}/${MovieTypesEnum.CARTOON}/${1}`,
    ...
    await handler.execute(
      new TelegramAdminBotSendNotificationNewModerationMovieCommand(MovieTypesEnum.FILM, 1),
    );
    await handler.execute(
      new TelegramAdminBotSendNotificationNewModerationMovieCommand(MovieTypesEnum.CARTOON, 1),
    );
  ...
  it('should not send notification if movie not found', async () => {
    const spy = jest.spyOn(telegramAdminBotService, 'sendHtmlMessage');

    try {
      await Promise.all([
        handler.execute(
          new TelegramAdminBotSendNotificationNewModerationMovieCommand(MovieTypesEnum.FILM, 999),
        ),
        handler.execute(
  ...
  it('should not send notification if movie type is unsupported', async () => {
    ...
    try {
      await handler.execute(
        new TelegramAdminBotSendNotificationNewModerationMovieCommand('type' as MovieTypesEnum, 1),
      );

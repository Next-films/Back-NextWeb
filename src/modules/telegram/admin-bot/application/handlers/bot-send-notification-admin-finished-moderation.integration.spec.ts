...
import {
  TelegramAdminBotSendNotificationAdminFinishedModerationCommand,
  TelegramAdminBotSendNotificationAdminFinishedModerationCommandHandler,
} from '@/telegram/admin-bot/application/handlers/bot-send-notification-admin-finished-moderation.handler';

describe('TelegramAdminBotSendNotificationAdminFinishedModerationCommandHandler (integration)', () => {
  let app: INestApplication;
  let handler: TelegramAdminBotSendNotificationAdminFinishedModerationCommandHandler;
  ...
  beforeAll(async () => {
    ...
    handler = app.get(TelegramAdminBotSendNotificationAdminFinishedModerationCommandHandler);
  ...
  it('should send notification about finished moderation task', async () => {
    ...
    const expectedData1: BotAdminFinishedModerationPayloadDto = {
      movieId: 1,
      title: `Movie 0`,
    ...
    const expectedData2: BotAdminFinishedModerationPayloadDto = {
      movieId: 1,
      title: `Movie 0`,
      telegramUsername: admin_tg_username,
      finishedAt: expect.any(String),
    ...
    await handler.execute(
      new TelegramAdminBotSendNotificationAdminFinishedModerationCommand(MovieTypesEnum.FILM, 1, 1),
    );
    await handler.execute(
      new TelegramAdminBotSendNotificationAdminFinishedModerationCommand(

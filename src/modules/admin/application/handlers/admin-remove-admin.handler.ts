...
export class AdminRemoveAdminCommand implements ICommand {
  constructor(
...
@CommandHandler(AdminRemoveAdminCommand)
export class AdminRemoveAdminCommandHandler
  implements
    ICommandHandler<
    ...
    >
{
  constructor(
    ...
    this.logger.setContext(AdminRemoveAdminCommandHandler.name);
  }

  async execute(
    command: AdminRemoveAdminCommand,
  ): Promise<AppNotificationResult<null, ErrorFieldExceptionDto | null>> {
    const { userId, currentUserId } = command;
    this.logger.log('Admin remove admin command', this.execute.name);
    ...
    try {
      ...
      await this.adminRepository.removeById(userId, queryRunner);

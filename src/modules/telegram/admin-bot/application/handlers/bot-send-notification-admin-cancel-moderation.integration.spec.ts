...
import {
  BotAdminCanceledModerationPayloadDto,
...
import {
  TelegramAdminBotSendNotificationAdminCancelModerationCommand,
  TelegramAdminBotSendNotificationAdminCancelModerationCommandHandler,
} from '@/telegram/admin-bot/application/handlers/bot-send-notification-admin-cancel-moderation.handler';

describe('TelegramAdminBotSendNotificationAdminCancelModerationCommandHandler (integration)', () => {
  let app: INestApplication;
  let handler: TelegramAdminBotSendNotificationAdminCancelModerationCommandHandler;
  ...
  beforeAll(async () => {
    ...
    handler = app.get(TelegramAdminBotSendNotificationAdminCancelModerationCommandHandler);
  ...
  it('should send notification about cancel moderation task', async () => {
    ...
    const expectedPayload: BotSendMessagePayloadDto = {
      ...
      template: ADMIN_BOT_TEMPLATES_NAME_ENUM.MOVIE_MODERATION_CANCELED_BY_ADMIN,
    };

    const expectedData1: BotAdminCanceledModerationPayloadDto = {
      movieId: 1,
      title: `Movie 0`,
      telegramUsername: admin_tg_username,
      rejectAt: expect.any(String),
      type: telegramAdminBotService.getMovieTypeString(MovieTypesEnum.FILM),
    };

    const expectedData2: BotAdminCanceledModerationPayloadDto = {
      movieId: 1,
      title: `Movie 0`,
      telegramUsername: admin_tg_username,
      rejectAt: expect.any(String),
    ...
    await handler.execute(
      new TelegramAdminBotSendNotificationAdminCancelModerationCommand(MovieTypesEnum.FILM, 1, 1),
    );
    await handler.execute(
      new TelegramAdminBotSendNotificationAdminCancelModerationCommand(

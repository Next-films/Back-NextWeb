...
import {
  ...
  IAdminBotNotificationCancelModerationStrategy,
...
@CommandHandler(TelegramAdminBotSendNotificationAdminCancelModerationCommand)
export class TelegramAdminBotSendNotificationAdminCancelModerationCommandHandler
  implements ICommandHandler<TelegramAdminBotSendNotificationAdminCancelModerationCommand, void>
{
  ...
  constructor(
    ...
    this.logger.setContext(
      TelegramAdminBotSendNotificationAdminCancelModerationCommandHandler.name,
    );
  ...
  async execute(
    command: TelegramAdminBotSendNotificationAdminCancelModerationCommand,
  ): Promise<void> {
    this.logger.log(`Send notification about admin cancel moderation command.`, this.execute.name);
    ...
    try {
      ...
      const data: BotAdminCanceledModerationPayloadDto = {
        movieId,
        title,
        telegramUsername: username,
        rejectAt: this.dateUtil.formatDateYyMmDdHhMm(new Date()),
  ...
  private getStrategyByType(
    type: MovieTypesEnum,
  ): IAdminBotNotificationCancelModerationStrategy | null {
    switch (type) {
